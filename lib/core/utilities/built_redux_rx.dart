...
A extends ReduxActions>(Iterable<Epic<V, B, A>> epics) {
  // ignore: close_sinks
  // ignore: omit_local_variable_types, close_sinks
...
typedef EpicHandler<V extends Built<V, B>, B extends Builder<V, B>,
A extends ReduxActions, P>
= Stream Function(Stream<Action<P>> stream, MiddlewareApi<V, B, A> mwApi);
...
A extends ReduxActions> {
  ...
  void add<T>(ActionName<T> actionName, EpicHandler<V, B, A, T> handler) {
    _epics.add((Stream<Action<dynamic>> action, MiddlewareApi<V, B, A> mwApi) =>
        handler(
